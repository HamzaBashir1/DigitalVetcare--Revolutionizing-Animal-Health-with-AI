...
const Dashboard = () => {
  const [sdoctor, setdoctor] = useState(null);
  ...
  const {
    ...
    loading,
    error,
  } = useFetchData(sdoctor ? `${Base_URL}/doctors/profile/me/${sdoctor?._id}` : null);
  ...
  return (
    <section>
      <div className="max-w-[1170px] px-5 mx-auto">
        ...
        {error && !data && !loading && <Error errMessage={error} />}
                  ...
                  <div>
                    <div className="flex items-center gap-4 mb-10">
                      ...
                      text-[12px] leading-4 lg:text-[16px] lg:leading-6 font-semibold"
                        ...
                        <p className="text-para font-[15px] lg:max-w-[390px]">{data?.bio}</p>
                      </div>
                    </div>
                    <DoctorAbout
                      name={data.name}
